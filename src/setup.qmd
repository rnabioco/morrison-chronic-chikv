...
type_cols <- new_1 <- set_names(new_1, names(type_cols))

type_cols["Stromal cells (DN)"] <- new_1["Monocytes"]
type_cols["Skeletal muscle"] <- new_1["DC"]
type_cols["Monocytes"] <- new_1["B cells"]
type_cols["B cells"] <- new_1["Skeletal muscle"]
type_cols["DC"] <- new_1["Stromal cells (DN)"]

type_cols["unassigned"] <- "#999999"

type_cols %>%
  qsave(here(params$ref_dir, "celltype_colors.qs"))
...
if (any(!names(t_clrs) %in% t_lvls)) {
  stop("Correct t_lvls, some labels are not included")
}
