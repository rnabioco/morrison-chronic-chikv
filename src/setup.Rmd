...
x <- so_t %>%
  ...
  classify_markers(
    ...
    filt       = Cd8a > 0.5 & Cd44 > 0.5 & Sell < 0.5,
    type_label = "CD8 Teff",
    clst_col   = final_clsts$t,
    type_col   = "t_type"
  ) %>%
  classify_markers(
    ...
    filt       = Cd4 > 0.5 & Cd44 > 0.5 & Sell < 0.5,
    type_label = "CD4 Teff",
    clst_col   = final_clsts$t,
    type_col   = "t_type"
  )
...
gns <- c("Sell", "Cd44", "Cd8a", "Cd4", "Trdc", "Foxp3", "Il7r")
...
save_objs(so_t, ob_dir = so_dir)
```

```{r "load objects"}
# Load Seurat objects
so     <- qread(here(so_dir, "so.qs"))
so_mac <- qread(here(so_dir, "so_mac.qs"))
so_t   <- qread(here(so_dir, "so_t.qs"))
