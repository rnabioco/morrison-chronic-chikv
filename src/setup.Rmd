  ...
  map(~ {
    qc <- .x@meta.data %>%
      ...
      mutate(
        ...
        qc_pass  = !discard & pct_mito < params$mito_max,
        .cell_id = Cells(.x)
      ) %>%
...
sobjs <- sobjs %>%
  map(~ {
    .x %>%
      ...
      mutate_meta(
        mutate,
        qc_pass = qc_pass & dbl_class != "Doublet"
        # qc_class = ifelse(
...
so <- so %>%
  ...
  ScaleData(assay = "RNA", vars.to.regress = "pct_mito") %>%
  cluster_RNA(
    assay      = "RNA",
    resolution = rslns,
    dims       = 1:50
...
sobjs %>%
  qsave(here(dirs$so_dir, "sobjs.qs"))

so_raw_df <- sobjs %>%
  map_dfr(~ {
    .x@meta.data %>%
      as_tibble(rownames = "cell_id") %>%
      mutate(cell_id = str_c(orig.ident, "_", cell_id))
  })

so_raw_df %>%
  write_tsv(here(dirs$so_dir, "so_raw.tsv.gz"))
...
so_df  <- read_tsv(here(dirs$so_dir, "so.tsv.gz"))
