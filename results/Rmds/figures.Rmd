...
params:
  ...
  ref_dir:      "ref"
  table_dir:    "results/tables/2024-01-22_4"          # Directory to write tables
...
# abs log2 fold change > 0.25 for 2/3 reps
...
mac_degs <- so_mac %>%
  find_conserved_markers(
    ...
    n_reps     = 2,
    filter_sig = FALSE,
    file_path  = here(dirs$table_dir, "mac_chikv_markers.tsv"),
...
info <- "
  ...
  Genes were filtered for those where the change in expression was in the same
  direction (up or down) for all replicates,
  an adjusted p value < 0.05 and
  an absolute log2 fold change > 0.25 for at least 2/3 replicates
"

mac_degs_sig <- mac_degs %>%
  filter(significant & all_same_direction)

mac_degs_sig %>%
  write_xlsx(
    info      = info,
    tab_var   = c("ident_1", "direction"),
    file_path = here(dirs$table_dir, "mac_chikv_markers.xlsx")
  )
```
...
mac_type_degs <- so_mac %>%
  find_conserved_markers(
    ...
    n_reps     = 2,
    filter_sig = FALSE,
    file_path  = here(dirs$table_dir, "mac_type_chikv_markers.tsv"),
    n_threads  = params$n_threads
  )

# Save significant markers as xlsx file
info <- "
  Markers were identified for mock- vs CHIKV-infected mice for each macrophage subset.
  Genes were filtered for those where the change in expression was in the same
  direction (up or down) for all replicates,
  an adjusted p value < 0.05 and
  an absolute log2 fold change > 0.25 for at least 2/3 replicates.
  Mitochondrial genes are not shown.
"

mac_type_degs_sig <- mac_type_degs %>%
  filter(
    significant & all_same_direction,
    !grepl("^mt-", gene)
  )
...
mac_type_degs_sig %>%
  write_xlsx(
    ...
    file_path = here(dirs$table_dir, "mac_type_chikv_markers.xlsx")
  )
```
...
info <- "
  ...
  Genes were included in they were upregulated for all replicates,
  and have an adjusted p value < 0.05 and
  log2 fold change > 0.25 for at least 2/3 replicates
"

t_type_degs_sig <- t_type_degs %>%
  filter(significant & all_same_direction & direction == "up")

t_type_degs_sig %>%
  write_xlsx(
    info      = info,
    tab_var   = "ident_1",
    file_path = here(dirs$table_dir, "t_type_markers.xlsx")
  )
```
...
t_degs_1 <- so_t %>%
  find_conserved_markers(
    ...
    n_reps     = 2,
    filter_sig = FALSE,
    file_path  = here(dirs$table_dir, "t_chikv_markers-1.tsv"),
    n_threads  = params$n_threads
  )
...
chikv_deg_file <- here(dirs$table_dir, "t_chikv_markers-2.tsv")
...
t_degs_2 <- read_tsv(chikv_deg_file)
...
chikv_deg_file <- here(dirs$table_dir, "t_type_chikv_markers.tsv")
...
t_degs_3 <- read_tsv(chikv_deg_file)
```
...
* Lining macrophage clusters were further assessed.
  The detection of this population is inconsistent for the mock-infected
  replicates (higher for r1, really low for r2, r3).
  This inconsistency is largely because doublets are being disproportionately
  marked/filtered from the lining r2 and r3 populations.
  The data were reprocessed after reducing the stringency of this step.
  This results in more consistent (and I think accurate) detection of the
  lining population, and there is a more reproducible reduction in this
  population in the CHIKV samples (which supports previous data).
* Plots are included showing expression of the Culemann et al. gene lists used
  for annotating the macrophage subsets.
* CHIKV+ cells were further assessed.
  Plots are included for the previous approach where macrophages clusters are
  split into CHIKV-high/low groups.
  The enrichment of CHIKV+ cells in specific populations was also assessed
  using a hypergeometric test.
  The populations with the highest fraction of CHIKV+ cells are lining and
  infiltrating macrophages.
  However, the p-values are not impressive.
  I'm going to see if I can come up with a better approach for presenting these
  results.
* Plots are included summarizing differentially expressed genes for mock vs
  CHIKV for all macrophages grouped together, and for each individual subset.
* There are two key background signals that are observable when looking at genes
  downregulated in the CHIKV samples. 
  * The mock samples are generally lower quality and have a higher proportion of
    mitochondrial counts, to account for this, mitochondrial genes have been
    omitted from the figures.
  * The mock samples also show higher expression of genes associated with
    skeletal muscle in all cell types (troponin, myosin genes).
    In 28 dpi datasets there are two clusters annotated as skeletal muscle,
    however, these cells are generally too large for the microfluidics used
    in the 10x platform.
    Many of these muscle cells are likely not fully intact and the mRNA
    originating from these cells likely contaminated the buffer during capture.
    There is no reason to believe this background signal is skewing our results,
    but this explains why troponin/myosin genes are differentially expressed 
    (for publication it would be reasonable to omit these genes and explain why). 
* Plots are included summarizing expression of MHC class II genes for
  mock vs CHIKV for each subset.
* Plots were regenerated for the Simmons et al. gene lists after adjusting
  QC filtering as described above.
...
* Plots are included summarizing differentially expressed genes for
  mock vs CHIKV for all T cells grouped together,
  and for CD4/CD8 T cells grouped together.
...
```{r "cell type Ifng 1", fig.width = 7, fig.height = 6, out.width = "65%"}
so %>%
  mutate_meta(
    mutate,
    treatment = fct_relevel(treatment, treats)
...
```{r "cell type Ifng 2", fig.width = 7, fig.height = 6, out.width = "65%"}
clrs <- type_cols
...
Genes were filtered for those where the change in expression was in the same
direction (up or down) for all replicates,
an adjusted p value < 0.05 and an absolute log2 fold change > 0.25
for at least 2/3 replicates
...
gns <- c("Nlrp3", "Il1b", "Tlr2")

if (!all(gns %in% gsea_gns)) stop("Not all genes are in GSEA terms")
...
```{r "Ag presentation plots", fig.width = 16, fig.height = 20}
# Set plot colors
u_clrs    <- grp_cols
u_clrs[1] <- lighten("#009E73", 0.2)
...
Top 60 differentially expressed genes in CHIKV-infected mice
...
Subsets with overlapping genes are connected by a
black line, bars show the number of genes.
...
upset_dat <- mac_type_degs_sig %>%
  split(.$direction) %>%
  imap(~ {
    .x %>%
      group_by(mac_type) %>%
      mutate(mac_type = str_c(mac_type, " (", label_comma()(n()), ")")) %>%
      group_by(gene) %>%
      summarize(mac_type = list(mac_type), .groups = "drop")
  })

# Format labels for number of cell types sharing gene
upset_key <- upset_dat %>%
  map(~ {
    .x %>%
      mutate(
        n_types = map_dbl(mac_type, length),
        lab     = str_c(gene, " (", n_types, ")")
      ) %>%
      select(-mac_type)
  })
    ...
    .x %>%
      ...
      imap(~ {
        ...
        dat <- so_mac %>%
          ...
          group_by(gene, lab, treatment, rep) %>%
          summarize(value = mean(value), .groups = "drop") %>%
          group_by(gene, lab) %>%
          mutate(value = as.numeric(scale(value))) %>%
          ungroup() %>%
...
the 28 differentially expressed genes included in the supplementary data.
...
```{r "lum gene plots", fig.width = 16, fig.height = 32}
# Set plot colors
u_clrs    <- grp_cols
u_clrs[1] <- lighten("#009E73", 0.2)
...
```{r "Ifng expression", fig.width = 7, fig.height = 6, out.width = "65%"}
so_t %>%
  mutate_meta(
    mutate,
    treatment = fct_relevel(treatment, treats)
...
Top 60 genes upregulated in CHIKV vs mock-infected mice are shown below, all T cells
were grouped together.

Upregulated genes were identified separately for each replicate and filtered as
described previously for the macrophage subsets.

```{r "all upreg T cells", fig.width = 12, fig.height = 4}
...
# Create heatmaps
asp_rat <- n_distinct(plt_dat$rep) / n_distinct(plt_dat$name)
...
Top 60 genes upregulated in T cells from CHIKV vs mock-infected mice are shown below.
