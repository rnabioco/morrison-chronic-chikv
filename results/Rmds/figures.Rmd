...
hlogs <- hfile %>%
  read_tsv() %>%
  distinct(gene, gene_mm)
```

```{r "mac DEGs", eval = FALSE}
# CHIKV+ clusters DEGS
...
```{r "t cell type markers", eval = FALSE}
...
# Filter DEGs
fc_clmns <- grep("_log2FC", colnames(degs), value = TRUE)
p_clmns  <- grep("_p_val_adj", colnames(degs), value = TRUE)

top_degs <- degs %>%
  rowwise() %>%
  mutate(
    min_fc   = min(!!!syms(fc_clmns)),
    max_padj = max(!!!syms(p_clmns))
  ) %>%
  ungroup() %>%
  filter(
    max_pval < 0.05,
...
simmons_genes <- simmons_genes %>%
  map(~ {
    .x %>%
      left_join(hlogs, by = c(Gene = "gene")) %>%
      filter(!is.na(gene_mm))
  })
```

```{r "simmons module scores", fig.width = 10, fig.height = 28, eval = FALSE}
...
so_mac <- qread(here(params$so_dir, "original_objects/so_mac_int.qs"))
...
plot_grid(plotlist = mod_plts, ncol = 1)

ggsave(
  here("results/2024-01-04_simmons_genes.png"),
  width = 10, height = 28, dpi = 300
)
```
