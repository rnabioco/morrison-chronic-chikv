...
gsea_terms <- mac_chikv_gsea@result %>%
...
Heatmaps show expression of the top 20 genes (arranged by maximum adjust p-value)
for each macrophage subset upregulated in CHIKV vs mock-infected samples.
Diamonds indicate genes that are significantly upregulated.

```{r "mac chikv type heatmaps", fig.width = 5.2, fig.height = 6}
# Set genes to plot
...
expr_dat <- mac_type_chikv_sig %>%
  filter(
    mac_type != "unassigned",
    direction == "up"
  ) %>%
  group_by(gene) %>%
  mutate(n = n_distinct(mac_type)) %>%
  ungroup() %>%
  
  mutate(
    mac_type = fct_relevel(mac_type, names(mac_typ_cols))
  ) %>%
  select(gene, mac_type, max_p_adj, n)

gns <- expr_dat %>%
  arrange(max_p_adj) %>%
  group_by(mac_type) %>%
  slice(1:20) %>%
  arrange(desc(n), max_p_adj, mac_type) %>%
  pull(gene) %>%
  unique()
...
dat %>%
  ...
  scale_fill_gradientn(colours = c("lightblue", "white", "#d7301f"), name = "z-score") +
  scale_x_discrete(labels = .simplify_sample()) +
...
Heatmaps show expression of the top 10 marker genes
(arranged by minimum fold change) for each macrophage subset.

```{r "mac type heatmaps", fig.width = 5.2, fig.height = 6.1}
# Set genes to plot
# * exclude unassigned macrophages
expr_dat <- mac_type_sig %>%
  filter(
    ident_1 != "unassigned"
  ) %>%
  rename(mac_type = ident_1) %>%
  arrange(desc(min_abs_fc)) %>%
  group_by(mac_type) %>%
  slice(1:10) %>%
  ungroup() %>%
  ...
  arrange(mac_type, max_p_adj) %>%
  select(mac_type, gene, max_p_adj)
...
dat <- so_mac %>%
  ...
  mutate(
    ...
    marker_type = fct_relevel(marker_type, names(mac_typ_cols))
  )
...
Bargraphs showing top GSEA disease ontology terms for genes upregulated when
comparing all mock vs all CHIKV-infected cells.
GSEA terms were filtered to remove broad terms that include >250 genes.

```{r "GSEA bargraph", fig.width = 8, fig.height = 4, out.width = "60%"}
# Bargraph showing top terms
# Filter to remove broad gene sets
n_terms <- 6
...
gsea_gns <- mac_chikv_gsea@result %>%
  dplyr::filter(
...
# Only plot genes that are upregulated in CHIKV for bulk macrophages
goi_lst <- goi_lst %>%
  map(~ {
    gns <- mac_chikv_sig %>%
      filter(gene %in% .x) %>%
...
for the subset.
The number of shared macrophage subsets is shown in parenthesis for each gene.

```{r "DEG upset plot", fig.width = 10, fig.height = 4}
# Calculate overlap between cell types
degs_dat <- mac_type_chikv_sig %>%
...
```{r "Ifng expression 2", fig.width = 5.75, fig.height = 3.75, out.width = "65%"}
so_t %>%
  create_celltype_plots_2(
    gn       = "Ifng",
    bx_clmn  = "t_type",
...
t_heats <- plt_dat %>%
  ...
  imap(~ {
    ...
    .x %>%
      mutate(name = fct_relevel(name, rev(gns_key[[.y]]))) %>%
...
t_typs %>%
  walk(~ {
    if (!.x %in% t_sig_3$t_type) return(NULL)
...
```{r "TENT5A EXPRESSION", fig.width = 8, fig.height = 10, eval = FALSE}
# n labels
gn <- "Tent5a"

plt_dat <- so %>%
  FetchData(c("cell_type", "chikv_grp", gn)) %>%
  mutate(
    chikv_grp = fct_relevel(chikv_grp, chikv_grps),
    type_grp  = str_c(cell_type, "-", chikv_grp),
  ) %>%
  arrange(chikv_grp) %>%
  mutate(type_grp = fct_inorder(type_grp))

lab_dat <- plt_dat %>%
  group_by(chikv_grp, type_grp, fill = chikv_grp) %>%
  summarize(n = n(), .groups = "drop") %>%
  mutate(lab = str_c(chikv_grp, "\nn = ", label_comma()(n)))

lab_dat <- set_names(lab_dat$lab, lab_dat$type_grp)

# boxplots
plt_dat %>%
  ggplot(aes(type_grp, Tent5a, fill = chikv_grp)) +
  geom_boxplot(outlier.size = 0.25) +
  facet_wrap(~ cell_type, scales = "free_x") +
  scale_fill_manual(values = grp_cols) +
  scale_x_discrete(labels = lab_dat) +
