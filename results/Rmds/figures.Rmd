...
all_mac_degs <- so_mac %>%
  find_conserved_markers(
    ...
    n_reps     = 2,
    filter_sig = FALSE,
    file_path  = here(dirs$table_dir, "mac_all_chikv_degs.tsv")
  )
```

```{r "mac subset DEGs"}
# Identify mock vs CHIKV DEGs for macroaphges subsets
# include genes with adjusted p-value < 0.05 and abs log2 fold change > 0.25 for
# 2/3 reps
chikv_deg_file <- here(dirs$table_dir, "mac_subset_chikv_degs.tsv")

if (!file.exists(chikv_deg_file)) {
  typs <- unique(so_mac$mac_type)
...
t_all_degs_1 <- so_mac %>%
  find_conserved_markers(
    ...
    n_reps     = 2,
    filter_sig = FALSE,
    file_path  = here(dirs$table_dir, "t_all_chikv_degs-1.tsv")
  )
