...
```{r "CELL TYPE ABUNDANCE", fig.width = 8, fig.height = 8}
## CLEAN THIS UP
...
prop_p <- prop_p %>%
  ...
  filter(cell_type != "unassigned") %>%
  arrange(FDR)

# Calculate cell proportions for plotting
prop_dat <- so@meta.data %>%
  group_by(treatment, rep, sample) %>%
  mutate(n = n()) %>%
  group_by(cell_type, treatment, rep, sample, n) %>%
  ...
  mutate(
    ...
    cell_type = fct_relevel(cell_type, as.character(prop_p$cell_type))
  )

# Create bar graphs
prop_bars <- prop_dat %>%
  filter(cell_type != "unassigned") %>%
  ggplot(aes(sample, prop_cells, fill = cell_type, color = cell_type, alpha = treatment)) +
  geom_col(width = 0.8) +
  ...
  facet_wrap(
    ~ cell_type,
    scales = "free"
    # labeller = as_labeller(mac_strip_labs, default = label_parsed)
  ) +
...
prop_p <- prop_p %>%
  ...
  ungroup() %>%
  rename(mac_type = BaselineProp.clusters) %>%
  filter(mac_type != "unassigned") %>%
  arrange(FDR)

# Calculate cell proportions for plotting
prop_dat <- so_mac@meta.data %>%
  group_by(treatment, rep, sample) %>%
  mutate(n = n()) %>%
  group_by(mac_type, treatment, rep, sample, n) %>%
  summarize(n_mac = n(), .groups = "drop") %>%
  mutate(
    prop_mac  = n_mac / n,
    treatment = fct_relevel(treatment, treats)
  ) %>%
  ...
  mutate(
    sample   = fct_inorder(sample),
    mac_type = fct_relevel(mac_type, as.character(prop_p$mac_type))
  )

# Create bar graphs
prop_bars <- prop_dat %>%
  filter(mac_type != "unassigned") %>%
  ggplot(aes(sample, prop_mac, fill = mac_type, color = mac_type, alpha = treatment)) +
  geom_col(width = 0.8) +
  ...
  scale_x_discrete(labels = simp_sam_labs) +
  scale_y_continuous(expand = expansion(c(0.05, 0.15))) +
  labs(y = "proportion of macrophages") +
  base_theme +
  theme(
    legend.position = "none",
    axis.title.x = element_blank(),
